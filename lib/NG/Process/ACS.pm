...
sub new {
  ...
  { my $type = $source->type;
    ...
    { $self->{"ACS"} = Net::Cisco::ACS->new(hostname => $source->hostname, username => $source->username, password => $source->password, ssl=> $source->ssl);
    }
  }
  bless $self, $class;
  return $self;
...
{ my $self = shift;
  ...
  { $groups{$accountgroup->name} = $accountgroup->id;
    $idgroups{$accountgroup->id} = $accountgroup->name;
  }
    ...
    { my $usergroup = $groups{$users{$user}->identityGroupName}; # Map ACS Name to ACS ID
      ...
      $self->{"DB"}->resultset('DsAcsUser')->create(
          { enabled => $users{$user}->enabled, description => $users{$user}->description, identitygroupname => $usergroup,
            ...
            dateexceeds => $users{$user}->dateExceeds, passwordtype => $users{$user}->passwordType, name => $users{$user}->name,
            status => $status, id => $users{$user}->id, created => $createdate, lastmodified => $moddate
          });
    }
    ...
    { my $usergroup = $groups{$users{$user}->identityGroupName};
      ...
      { $acsusers{$user}->update(
          { enabled => $users{$user}->enabled, description => $users{$user}->description, identitygroupname => $usergroup,
            passwordneverexpires => $users{$user}->passwordNeverExpires, dateexceedsenabled => $users{$user}->dateExceedsEnabled,
            dateexceeds => $users{$user}->dateExceeds, passwordtype => $users{$user}->passwordType, name => $users{$user}->name,
            status => $status, id => $users{$user}->id, lastmodified => $users{$user}->lastModified
          });
      }
...
{ my $self = shift;
  ...
  { my $decpassword;
    ...
    $self->{"ACS"}->update($user) if $account->status eq $status_changed;
    $self->{"ACS"}->create($user) if $account->status eq $status_created;
    $self->{"ACS"}->delete($user) if $account->status eq $status_deleted;
  }
}
...
{ my $self = shift;
  my $identitygroups = $self->{"ACS"}->identitygroups;
  ...
  { if (!$acsidentitygroups{$identitygroup})
    { my $status = $status_imported;
      $self->{"DB"}->resultset('DsAcsIdentitygroup')->create(
          { description => $identitygroups{$identitygroup}->description, name => $identitygroups{$identitygroup}->name,
            status => $status, id => $identitygroups{$identitygroup}->id
          });
    }
...
{ my $self = shift;
  ...
  { my $identitygroup =
    ...
    $self->{"ACS"}->update($identitygroup) if $group->status eq $status_changed;
    $self->{"ACS"}->create($identitygroup) if $group->status eq $status_created;
    $self->{"ACS"}->delete($identitygroup) if $group->status eq $status_deleted;
  }
}
